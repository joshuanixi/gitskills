...
class MWS(object):
	...
	def make_request(self, extra_data, method="GET", **kwargs):
		...
		try:
			...
			response = request(method, url, data=kwargs.get('body', ''), headers=headers,timeout=60)
			# response = request(method, url, data={}, headers=headers)
	...
	def calc_signature(self, method, request_description):
		...
		return base64.b64encode(hmac.new(self.secret_key.encode('utf8'), sig_data.encode('utf8'), hashlib.sha256).digest()).decode()
