...
from mws import mws
import get_unshipped_orders

def confirm_orders(confirm_file):
	...
	#confirm_file_path=newlocal.get_path(root_path,['src','upload','confirm'])+confirmfile

    
    if (os.stat(confirm_file).st_size == 0):
    	print('please check confirm file, not exist or is empty')
    	time.sleep(500)
...
confirm_orders(needconfirmorder())